...
path_texture_files = 'textureFiles'; % Change this if the folder is located elsewhere.
...
figure(6)
sgtitle('Vector of noise levels')
TB = 2; % testimage nr.
vec_N = [1,5,10,25,50];
...
plot(vec_N,E_N,'bo')
plot(vec_N,E_N,'r-')
title('Reconstruction with vector of noise levels')
xlabel('Noise level / %')
ylabel('Reconstruction error percentage')

%% Vector of samplings
figure(8)
sgtitle('Vector of samplings')
...
plot(vec_S,E_S,'bo')
plot(vec_S,E_S,'r-')
title('Reconstruction with vector of samplings')
xlabel('Sampling share')
ylabel('Reconstruction share')
...
for j = 1:length(vec_N)
    for k = 1:length(vec_S)
        ...
        if k == 1
            ylabel(sprintf('%d%% noise',vec_N(j)),'FontSize',12,...
...
% Resizing the lateral slices, otherwise the image is too thin to see anything
...
%% Ukendte objekter
A = load('Data\A.mat'); A = A.A;
A = recon_volume(A,1:size(A,3));
[AO1,AO2,AO3] = ortho_slices(A,130,130,130);

B = load('Data\B.mat'); B = B.B;
B = recon_volume(B,1:size(B,3));
[BO1,BO2,BO3] = ortho_slices(B,60,60,35);

for k = ['A','B']
    figure(13 + k - 'A'), sgtitle(['Unknown data: object ' k])
    for j = 1:3
        subplot(2,2,j)
        O_n = eval([k 'O' num2str(j)]);
        if j == 3
            imshow(imrotate(abs(O_n),90),[]);
        else
            imshow(abs(O_n),[]);
        end
